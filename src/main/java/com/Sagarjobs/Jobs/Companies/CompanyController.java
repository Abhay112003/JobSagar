...
//call compnay Registration page
 ...
 {
     model.addAttribute("comdata",new CompanyEntity());
                        ...
                        Model model) throws Exception
{
        ...
        if(companyRepositary.findByEmail(validate.getEmail()).isPresent())
        {
...
//             }
                ...
                emailservice.sendmail(validate.getEmail(), otp,validate.getName());
     ...
     try {
      ...
      if(currtime.isAfter(time))
      {
          ...
          session.removeAttribute("otp");
           ...
           emailservice.RegistrationSuccess(job.getEmail(),job.getName());
...
//Company html page call to edit
 ...
 //  application
    ...
    public String ForgetOtp(HttpSession session,@RequestParam("email") String email,Model model)throws Exception
    {
        ...
        emailservice.sendmail(email,OTP,company.get().getName());
    ...
    public String ResendOtp(HttpSession session,Model model) throws Exception
    {
        ...
        emailservice.sendmail(company1.getEmail(),OTP,company1.getName());
        session.setAttribute("OTP",OTP);
        model.addAttribute("comdata",company);
    ...
    public String VerifyOtp(@RequestParam("otp") String otp,HttpSession session,Model model)
    {
        ...
        if(!otp.equals(Otp)) {
            model.addAttribute("error", "Enter Correct OTP");
            return "Company/VerifyOtp";
        }
        CompanyEntity company1=company.get();
      ...
      public String UpdatePassword(@RequestParam("password") String password, HttpSession session, Model model)
     {
       ...
       String email=company.getEmail();
        if(company==null && !email.equals(null))
        {
     ...
     //Profile Update
    @RequestMapping("/Companyprofileedit")
    public String Update(HttpSession session,Model model)
    {
         CompanyEntity company= (CompanyEntity) session.getAttribute("comdata");
         model.addAttribute("comdata",company);
         return "Company/UpdateProfile";
    }
    @PostMapping("/Company/Profilepicupdate")
    public String UpdateProfile(@RequestParam("picture") MultipartFile file,HttpSession session,Model model)
    {
        try {
            CompanyEntity company = (CompanyEntity) session.getAttribute("comdata");
            if(file.isEmpty())
            {
                model.addAttribute("comdata",company);
                model.addAttribute("profileerror","Profile pic not selected");
                return "Company/UpdateProfile";
            }
            companySaveService.saveimage(file.getBytes(), company);
            model.addAttribute("comdata", company);
            model.addAttribute("profileUpdated", "Update");
            return "Company/UpdateProfile";
        } catch (Exception e) {
    ...
    @PostMapping("/CompanyDataSaved")
    public String CompanydataSaved(@Valid @ModelAttribute("comdata")CompanyEntity data ,BindingResult result, HttpSession session,Model model)
    {
        if(result.hasErrors())
        {
            return "/Company/UpdateProfile";
        ...
        CompanyEntity company1=companyRepositary.findById(id).orElse(null);
        System.out.println("Company is "+company1);
        if(company1==null) {
          model.addAttribute("error","Id Not Found");
        }
        company1.setEmail(data.getEmail());
        company1.setContact(data.getContact());
        company1.setName(data.getName());
        company1.setDescription(data.getDescription());
        company1.setAddress(data.getAddress());
        companyRepositary.save(company1);
        model.addAttribute("comdata",company1);
        session.setAttribute("comdata",company1);
        return "Company/Companydashboard";
    }
